...
class Evaluator(BaseEvaluator):
    ...
    def convert_batch_to_inputs(self, batch):
        if self.cfg.model_type=="paie" or self.cfg.model_type=="t5p":
            inputs = {
                ...
                'target_info':              None,
                'arg_list':       batch[9],
                "event_trigger": batch[16], #______________
                "dec_prompt_text": batch[17],  # ______________
                'target_info': batch[6],
            }
    ...
    def collect_fn(self, outputs_list, named_v, batch):   
        ...
        for i in range(bs):
            ...
            for arg_role in named_v["arg_roles"][i]:
                if arg_role not in predictions.keys():
                    continue
    ...
    def predict(self):
        ...
        if self.cfg.model_type=='paie' or self.cfg.model_type=='bert':
            pred_list = []
            for s in range(0, len(self.record["full_start_logit_list"]), self.cfg.infer_batch_size):
                ...
                pred_list.extend(sub_max_locs) # location-identification
        ...
        for metric, eval_fn in self.metric_fn_dict.items():
            ...
            if metric == "span":
                if self.set_type == "DEV":
                    wandb.log({("dev-Classification"):perf_c['f1']})
                    wandb.log({("dev-Identification"): perf_i['f1']})
                if self.set_type == "TEST":
                    wandb.log({("test-Classification"): perf_c['f1']})
                    wandb.log({("test-Identification"): perf_i['f1']})
            elif metric == "head":
                if self.set_type == "DEV":
                    wandb.log({("Head-dev-Classification"): perf_c['f1']})
                if self.set_type == "TEST":
                    wandb.log({("Head-test-Classification"): perf_c['f1']})


        logger.info("------------ratio_loss: " + str(self.model.get_loss()))
