...
from transformers import BartTokenizerFast as tob

all_temp_arg = {'instrument': 'What', 'killer': 'Who', 'victim': 'Who', 'place': 'Where', 'artifact': 'What',
                'vehicle': 'What', 'transporter': 'Who', 'origin': 'Where', 'hidingplace': 'Where', 'attacker': 'Who',
                'target': 'Who', 'recipient': 'Who', 'communicator': 'Who', 'crime': 'What', 'jailer': 'Who',
                'detainee': 'Who', 'participant': 'Who', 'injurer': 'Who', 'placeofemployment': 'Where',
                'employee': 'Who', 'beneficiary': 'Who', 'giver': 'Who', 'investigator': 'Who', 'defendant': 'Who',
                'violator': 'Who', 'otherparticipant': 'Who', 'destination': 'Where', 'preventer': 'Who',
                'passenger': 'Who', 'money': 'What', 'prosecutor': 'Who', 'judgecourt': 'Who', 'voter': 'Who',
                'candidate': 'Who', 'governmentbody': 'What', 'law': 'What', 'extraditer': 'Who',
                'fireexplosionobject': 'What', 'territoryorfacility': 'What', 'gpe': 'What', 'founder': 'Who',
                'executioner': 'Who', 'artifactmoney': 'What', 'retreater': 'Who', 'manufacturer': 'Who',
                'observer': 'Who', 'observedentity': 'What', 'spy': 'Who', 'ballot': 'What', 'destroyer': 'Who',
                'granter': 'Who', 'demonstrator': 'Who', 'inspector': 'Who', 'inspectedentity': 'What',
                'deceased': 'Who', 'yielder': 'Who', 'damager': 'Who', 'result': 'What', 'rejecternullifier': 'Who',
                'crashobject': 'What', 'driverpassenger': 'Who', 'damagerdestroyer': 'Who', 'surrenderer': 'Who',
                'monitoredentity': 'Who', 'monitor': 'Who'}
...
class InputFeatures(object):
    ...
    def set_gt(self, model_type, dset_type):
        ...
        elif "paie" or "bert" in model_type:
            for k, v in self.target_info.items():
...
class ArgumentExtractionDataset(Dataset):
    ...
    def collate_fn(batch):
        ...
        dec_prompt_text = [f.dec_prompt_texts for f in batch]
        ...
        return enc_input_ids, enc_mask_ids, \
            ...
            start_position_ids, end_position_ids, event_trigger, dec_prompt_text
...
class MultiargProcessor(DSET_processor):
    ...
    def set_dec_input(self):
        ...
        if self.args.model_type == "base":
            self.arg_query = True
        elif "paie" or "bert" in self.args.model_type:
    ...
    def convert_examples_to_features(self, examples, alist=None):
        ...
        for example_idx, example in enumerate(examples):
            ...
            for arg_temp in arg_list:
                count = []
                for matching_result in re.finditer(r'\b' + re.escape(arg_temp) + r'\b', dec_prompt_text.split('.')[0]):
                    char_idx_s, char_idx_e = matching_result.span()
                    count.append(char_idx_e)
                for item in reversed(count):
                    dec_prompt_text = dec_prompt_text[:item] + " <mask>" + dec_prompt_text[item:]
            char_idx_s = dec_prompt_text.find(",end")
            dec_prompt_text = dec_prompt_text[:char_idx_s] + "in " + event_trigger[0] + " ,end"
            enc_text = enc_text + "<s></s>" + dec_prompt_text
            ...
            #     if enc.char_to_token(char_idx_s) is None:
            #         t = 1
            #         while t < len(old_tok_to_char_index):
            #             if enc.token_to_chars(t)[0] == char_idx_s:
            #                 new_tok_s = t
            #                 break
            #             t += 1
            #     else:
            #         new_tok_s = enc.char_to_token(char_idx_s)
            #     if enc.char_to_token(char_idx_e) is None:
            #         t = 1
            #         while t < len(old_tok_to_char_index):
            #             if enc.token_to_chars(t)[1] == char_idx_e:
            #                 new_tok_e = t
            #                 break
            #             t += 1
            #     else:
            ...
            if self.prompt_query:
                ...
                for arg_temp in arg_list:
                    count = []
                    for matching_result in re.finditer(r'\b' + re.escape(arg_temp) + r'\b',
                ...
                if dec_prompt_text:
                    ...
                    assert len(
                        dec_prompt_ids) <= self.args.max_prompt_seq_length, f"\n{example}\n{arg_list}\n{dec_prompt_text}"
            ...
            # ____________
            # dec_prompt_text_list = []
            # dec_prompt_ids_list = []
            # dec_prompt_mask_ids_list = []
            # if self.prompt_query:
            #     dec_prompt_text = prompts[event_type].strip()
            #     if dec_prompt_text:
            #         for arg_temp in arg_list:
            #             global all_temp_arg
            #             # dec_prompt_text_temp = dec_prompt_text + ". " + all_temp_arg[arg_temp] + " is the " + arg_temp + " in " + event_trigger[0] + " ? "
            #             dec_prompt_text_temp = dec_prompt_text + ". " + "<mask>" + " is the " + arg_temp + " in " + event_trigger[0]
            #             dec_prompt = self.tokenizer(dec_prompt_text_temp)
            #             dec_prompt_ids, dec_prompt_mask_ids = dec_prompt["input_ids"], dec_prompt["attention_mask"]
            #             assert len(dec_prompt_ids)<=self.args.max_prompt_seq_length, f"\n{example}\n{arg_list}\n{dec_prompt_text_temp}"
            #             while len(dec_prompt_ids) < self.args.max_prompt_seq_length:
            #                 dec_prompt_ids.append(self.tokenizer.pad_token_id)
            #                 dec_prompt_mask_ids.append(self.args.pad_mask_token)
            #
            #             dec_prompt_text_list.append(dec_prompt_text_temp)
            #             dec_prompt_ids_list.append(dec_prompt_ids)
            #             dec_prompt_mask_ids_list.append(dec_prompt_mask_ids)
            #     else:
            #         raise ValueError(f"no prompt provided for event: {event_type}")
            # else:
            #     dec_prompt_text, dec_prompt_ids, dec_prompt_mask_ids = None, None, None
            ...
            # for i, (dec_prompt_text, dec_prompt_ids, dec_prompt_mask_ids) in \
            #         enumerate(zip(dec_prompt_text_list, dec_prompt_ids_list, dec_prompt_mask_ids_list)):
            #
            #     features.append(
            ...
            #                         enc_text, enc_input_ids, enc_mask_ids,
            #                         dec_prompt_text, dec_prompt_ids, dec_prompt_mask_ids,
            #                         arg_quries, {arg_list[i]:arg_joint_prompt[arg_list[i]]}, target_info,
            ...
            # for i in arg_joint_prompt.keys():
            #     features.append(
            #         InputFeatures(example_id, feature_idx,
            #                       event_type, event_trigger,
            #                       enc_text, enc_input_ids, enc_mask_ids,
            #                       dec_prompt_text, dec_prompt_ids, dec_prompt_mask_ids,
            #                       arg_quries, {i:arg_joint_prompt[i]}, target_info,
            #                       old_tok_to_new_tok_index=old_tok_to_new_tok_index, full_text=example.full_text,
            #                       arg_list=arg_list
            #                       )
            #     )
