...
from transformers import BartConfig, BartTokenizerFast, BertConfig, BertTokenizerFast, BertTokenizer, DistilBertConfig, DistilBertTokenizerFast, T5TokenizerFast, T5Config
...
MODEL_CLASSES = {
    ...
    't5p' : (T5Config, T5P, T5TokenizerFast),
    'base': (BartConfig, BartSingleArg, BartTokenizerFast),
    # "bert": (BertConfig, BertArg, BertTokenizerFast),
    # "distilbert": (DistilBertConfig, BertArg, DistilBertTokenizerFast),
}


def build_model(args, model_type, strategy=2):
    ...
    if args.inference_only:
        ...
        # scheduler = get_cosine_schedule_with_warmup(optimizer, num_warmup_steps=args.max_steps*args.warmup_steps, num_training_steps=args.max_steps, num_cycles=5)
        scheduler = get_cosine_with_hard_restarts_schedule_with_warmup(optimizer, num_warmup_steps=args.max_steps*args.warmup_steps, num_training_steps=args.max_steps, num_cycles=strategy)
